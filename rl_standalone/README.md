# rl_standalone — rodent locomotion motor model, standalone
...
```bash
uv run python rl_standalone/make_assets.py        # trains SimpleTrans on parent loco data + bundles it (imports canvas)
```

This trains the simplified transition (~3 min) on the parent repo's locomotion data with the frozen tokenizer, and
writes `assets/motor_standalone.pt`. It is the ONLY file here that reaches outside `rl_standalone`.
...
```bash
uv run python rl_standalone/drive.py --seconds 16 --render     # go straight / walk in cycles + path plot
uv run python rl_standalone/waypoint.py --shape square --render  # closed-loop reach 4 waypoints (also: star, zigzag)
```
