...
This is the ONE file here that reaches outside demo_b: it imports `canvas` (via the exploration harness)
for the training data + frozen tokenizer. The runtime (models/rollout/drive/waypoint/foot_metrics) imports nothing
outside demo_b. Run once from the CANVAS repo root:

    uv run python demo_b/make_assets.py [--steps 16000]
