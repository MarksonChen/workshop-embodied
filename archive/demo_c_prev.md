...
_Created 2026-07-18 and archived 2026-07-21. Companion to
[WORKSHOP_PLAN.md](../ref/docs/WORKSHOP_PLAN.md). This was an early proposal and
is superseded by the accepted [Demo H](../ref/docs/demo_h.md). Historical assets:
[PROJECT_STATE.md](../ref/docs/PROJECT_STATE.md)._
...
LAPO ([ref/repos/LAPO](../ref/repos/LAPO)) discovers latent actions from **action-free**
...
- **#1 technical risk — RESOLVED (2026-07-18 gradient-path spike).** `jax.grad`
  ...
  **Fix: forward-mode autodiff (`jax.jacfwd`)** — it differentiates through `while_loop`, and
  ...
  mocap transitions.)
- **Optional shaping (stack-native):** **HILP** ([ref/repos/HILP](../ref/repos/HILP), JAX/Flax)
  to give the intention a temporal-distance / directional structure — a smoother,
  goal-directed manifold.
- **Lower-risk alternative (Role A):** **MTM** ([ref/repos/mtm](../ref/repos/mtm)) state-only
...
- Warm-start the high-level policy from Stage 1 (BC on intentions and/or the encoder); PPO:
  ...
  warm-start.**
- **Borrow from TD-MPC2** ([ref/repos/tdmpc2](../ref/repos/tdmpc2); read in code,
...
[ref/papers/WorldModel.pdf](../ref/papers/WorldModel.pdf) (*World Model for Robot Learning: A
...
|---|---|---|
| [`LAPO`](../ref/repos/LAPO) | PyTorch | Stage-1 scaffold (action-free latent-action = intention discovery) — **port to JAX** |
| [`HILP`](../ref/repos/HILP) | JAX/Flax | optional intention shaping (temporal-distance directional latent) |
| [`mtm`](../ref/repos/mtm) | PyTorch | fallback Role-A representation warm-start |
| [`tdmpc2`](../ref/repos/tdmpc2) | PyTorch | contrast exhibit + borrowable self-predictive aux loss |
...
1. Full Stage-1 intention-IDM on the merged locomotion bouts. Data gotchas
   ([dataset.md](../ref/docs/dataset.md)): **merge bouts** (label flicker shreds raw runs), **finite-diff
