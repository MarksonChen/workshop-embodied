...
from workshop.part2.config import (
    FEATURE_CONTRACT_VERSION,
    LEGACY_FEATURE_CONTRACT_VERSION,
)
from workshop.part2.core.artifacts import sha256
from workshop.part2.core.jax_model import (
    causal_conv,
    layer_norm,
    linear,
    sinusoidal_positions,
)
from workshop.part3.config import OUT, PRIOR_CONTROL_LIMIT, PriorConfig


DEFAULT_PRIOR = OUT / "prior_jax.npz"


@dataclass(frozen=True)
class BodyActionPrior:
    tokenizer: dict
    predictor: dict
    action_decoder: dict
    ...
    def predict_plan(self, history, raw_command):
        ...
        return linear(
            hidden, self.predictor["output.2.weight"], self.predictor["output.2.bias"]
        )
    ...
    def action_mean(self, raw_feature, plan, previous_control, phase, raw_command):
...
def load_prior(path: Path = DEFAULT_PRIOR) -> BodyActionPrior:
    path = Path(path)
    if not path.is_file():
        raise FileNotFoundError(
            f"missing JAX prior {path}; run `python -m workshop.part3.export`"
        )
    with np.load(path) as archive:
        metadata = json.loads(str(archive["metadata_json"]))
        if metadata.get("schema") != "workshop-part3-jax-prior-v1":
        ...
        if feature_contract != FEATURE_CONTRACT_VERSION:
            raise ValueError(
                f"prior feature contract {feature_contract!r}; expected {FEATURE_CONTRACT_VERSION!r}"
            )
    ...
    return BodyActionPrior(
        metadata=metadata, artifact_sha256=sha256(path), **groups, **constants
    )
