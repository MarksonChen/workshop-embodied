...
from workshop.part3.core.model import FeedbackActionDecoder
from workshop.part3.pretrain import (
    DEVICE,
    _action_metrics,
    _closed_loop_action_metrics,
    _state_mse,
)
from workshop.part3.core.model import state_action_windows


def load_models(path: Path):
    checkpoint = torch.load(path, map_location=DEVICE, weights_only=False)
    if checkpoint.get("schema") != "workshop-part3-prior-v1":
...
def evaluate(checkpoint_path: Path, dataset_root: Path, split: str) -> dict:
    ...
    report = {
        "schema": "workshop-part3-prior-evaluation-v1",
        "split": split,
