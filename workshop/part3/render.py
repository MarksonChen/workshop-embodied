...
from workshop.part3.config import FPS
...
def _load_qps(env, trace_path: Path):
    ...
    def make_qp(prefix: str, index=None):
        ...
        select = (
            (lambda value: value) if index is None else (lambda value: value[index])
        )
        qp = base.replace(
...
class _ReusableRenderer:
    ...
    def render(self, qp) -> np.ndarray:
        ...
        pixels = self.scene.get_camera_image(self.instances, light, camera).rgb
        return np.asarray(pixels, dtype=np.uint8).reshape(self.height, self.width, -1)
    ...
    *,
    ...
    for metrics_path in map(Path, metrics_paths):
        ...
        for source_row in report["speeds"]:
            row = dict(source_row)
            row["group_label"] = report.get("label", "policy")
            rows.append(row)
    if not rows:
    ...
    with (
        ...
        imageio.get_writer(output, fps=FPS / frame_stride, quality=8) as writer,
    ):
