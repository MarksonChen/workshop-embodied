...
from workshop.part1.environment import FetchV2, RawKeyVmapWrapper
...
from workshop.part3.core.artifacts import load_policy_checkpoint
...
from workshop.part3.data.contract import DATASET_VARIANT, DEFAULT_ROOT
from workshop.part3.data.loader import load_split
from workshop.part3.core.control import PriorFetchRun
from workshop.part3.core.control import (
    compute_plans,
...
from workshop.part3.core.control import BatchedPlanWrapper
...
def reference_summary(
    ...
    test = load_split("test", dataset_root, expected_variant=dataset_variant)
...
def make_environment(prior, target_speed=TARGET_SPEED_FETCH):
    environment = FetchV2(
        PriorFetchRun(
            ...
            speed_width=max(target_speed / 3.0, 1e-3),
        )
    )
...
def make_actor(checkpoint: Path, prior):
...
def rollout_policy(checkpoint, prior, seeds, steps, target_speed=TARGET_SPEED_FETCH):
    ...
    construction_speed = float(target_speeds if scalar_target else target_speeds[0])
    environment = make_environment(prior, construction_speed)
    action_fn, parameter_fn = make_actor(checkpoint, prior)
...
def summarize(stream, seeds, steps, target_speed=TARGET_SPEED_FETCH):
    ...
    for episode_index, seed in enumerate(seeds):
        ...
        if complete:
            ...
            gait_values = {
                f"gait_{name}": float(np.mean(values)) for name, values in gait.items()
            }
        else:
        ...
        episode = {
            ...
            "metrics": {
                ...
                "upright_mean": float((upright[:, episode_index] * mask).sum() / count),
                "action_energy": float(
                ...
                "action_saturation_fraction": float(
                    (
                        (np.abs(action[:, episode_index]) >= 0.999).mean(axis=-1) * mask
                    ).sum()
                    / count
                ),
...
def main() -> None:
    ...
    save_trace(args.output.parent / "policy_trace.npz", initial, stream)
    report = {
        "schema": "workshop-part3-evaluation-v1",
        ...
        "aggregate": summary,
        "episodes": episodes,
        "claim_boundary": "Compare beta values without using gait metrics as rewards.",
    }
