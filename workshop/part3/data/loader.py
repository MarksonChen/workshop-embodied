    ...
    *,
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"missing Part 3 dataset at {path}; run `python -m workshop.part3.data.project`"
        )
    ...
    if not manifest.get("complete_release", False):
        raise ValueError("prior training requires a complete Part 3 release")
    if manifest.get("variant") != expected_variant:
        raise ValueError(
            f"unexpected Part 3 dataset variant {manifest.get('variant')!r}; "
            f"expected {expected_variant!r}"
        )
    if manifest.get("fields") != {name: list(shape) for name, shape in FIELDS.items()}:
        raise ValueError("Part 3 manifest fields differ from the frozen contract")
    if manifest.get("dtypes") != DTYPES:
        raise ValueError("Part 3 manifest dtypes differ from the frozen contract")
