...
from workshop.part2.core.artifacts import guard_derived_release_output, sha256
from workshop.part2.core.motion import hindsight_command, yaw_from_quaternion
from workshop.part2.config import FEATURE_CONTRACT_VERSION
from workshop.part2.core.motion import trajectory_features
from workshop.part2.core.motion import contact_flags
from workshop.part2.core.motion import fetch_feet
...
class ExactFetchProjector:
    ...
    def _initial_qp(
        ...
        world_offset = jax.vmap(jax.vmap(math.rotate, in_axes=(0, None)))(
        ...
        def step(qp, frame):
            ...
            control = jnp.clip(
                (
                    ...
                    + PD_VELOCITY_GAIN * (reference_rates[:, frame] - velocity)
                )
                / TORQUE_STRENGTH,
                -1.0,
                1.0,
    ...
    def project(self, arrays: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
        ...
        realized_contact = np.concatenate((contact[:, :1], contact), axis=1).astype(
            np.uint8
        )
        ...
        maximum_planar_speed = (
            np.linalg.norm(np.diff(realized_root[..., :2], axis=1) * FPS, axis=-1)
            .max(axis=1)
            .astype(np.float32)
        )
        realized_yaw = yaw_from_quaternion(realized_quaternion)
        maximum_yaw_rate = (
            np.abs(np.diff(realized_yaw, axis=1) * FPS).max(axis=1).astype(np.float32)
        )
        ...
        return {
            ...
            "reference_root_quaternion": padded["root_quaternion"][:count].astype(
                np.float32
            ),
            ...
            "requested_actuator_torque": (-TORQUE_STRENGTH * control).astype(
                np.float32
            ),
...
def project_release(
    ...
    *,
    ...
    if output_root.resolve() == DEFAULT_ROOT.resolve() and (
        ...
        raise ValueError(
            "partial or experimental projection requires a distinct output root"
        )
    ...
    if actual_parent_variant != parent_variant:
        raise ValueError(
            f"expected Part 2 {parent_variant!r}, got {actual_parent_variant!r}"
        )
    ...
    manifest = {
        ...
        "derivation": {
            ...
            "code_sha256": {
                ...
                "part3/data/project.py": sha256(Path(__file__).resolve()),
            },
        },
        "gates": {
...
def main() -> None:
    ...
    print(
        json.dumps(
            {
                key: manifest[key]
                for key in ("counts", "global_pass_rate", "build_seconds")
            },
            indent=2,
        )
    )
