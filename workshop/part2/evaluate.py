...
def likelihood_report(
    ...
    shuffled_logp = predictor.log_prob(history, future[permutation], command, sigma)
    ...
    for offset in offsets:
        ...
        counterfactual[:, :2] = direction[central] * (requested * scale)[:, None]
        normalized = (
            torch.as_tensor(counterfactual, device=device) - command_mean
        ) / command_std
        relative.append(
            predictor.log_prob(history[central], future[central], normalized, sigma)
            .mean()
            .item()
        )
    relative = np.asarray(relative, np.float32)
    return {
        "evaluation_contract": "workshop-part2-likelihood-v1",
        "n_windows": int(len(real_logp)),
        "mean_logp": float(real_logp.mean()),
        "shuffled_future_mean_logp": float(shuffled_logp.mean()),
        "real_minus_shuffled_logp": float(real_logp.mean() - shuffled_logp.mean()),
...
def fixed_seed_indices(dataset, count: int = EVAL_SEEDS) -> list[int]:
    ...
    if len(selected) != count:
        raise ValueError(
            f"only found {len(selected)} distinct validation seed sessions"
        )
    return selected
...
def rollout_report(checkpoint, config, tokenizer, predictor, train, validation) -> dict:
    ...
    for seed_index in seed_indices:
        ...
        for requested_speed in EVAL_SPEEDS:
            ...
            forward_equivalent = trailing_mean(
                features[:, SL["root_velocity"][0]] * COMMAND_HORIZON_SECONDS / scale
            )
    ...
    monotonic_violation = float(
        np.maximum(realized[:, :-1] - realized[:, 1:], 0).mean()
    )
    pause_fraction = float(np.mean([row["low_speed_dwell_fraction"] for row in rows]))

    objective = (
        tracking_mae / 0.15 + 0.5 * pause_fraction + 2.0 * monotonic_violation / 0.05
    )
    return {
        "evaluation_contract": "workshop-part2-rollout-v1",
    ...
    report.update(
        {
            ...
            "prediction_skill_over_persistence": prediction["skill_over_persistence"],
