...
from ..core.motion import trajectory_features
from .contract import DEFAULT_ROOT, DYNAMIC_ROOT, REPOSITORY_ID, SCHEMA_VERSION
...
def load_manifest(root: Path = DYNAMIC_ROOT, *, download: bool = False) -> dict:
    root = Path(root)
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"missing dataset at {path}; run `python -m workshop.prepare_data --download`"
        )
    ...
    if not manifest.get("complete_release", False):
        raise ValueError(
            "Part 2 canonical training requires a complete 38-session release"
        )
