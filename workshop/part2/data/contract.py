...
DEFAULT_ROOT = Path(
    os.environ.get(
        "WORKSHOP_RETARGETED_DATA",
        Path(__file__).resolve().parents[2] / "data" / "retargeted",
    )
)
DYNAMIC_ROOT = Path(
    os.environ.get(
        "WORKSHOP_PART2_DATA",
        Path(__file__).resolve().parents[2] / "data" / "part2",
    )
)
...
SESSION_TO_SPLIT = {
    session: split for split, sessions in SPLIT_SESSIONS.items() for session in sessions
}
