...
from workshop.part2.core.motion import hindsight_commands
from workshop.part2.config import FEATURE_CONTRACT_VERSION, FPS, JOINT_LIMIT
from workshop.part2.core.motion import fetch_feet, world_feet
from workshop.part2.data.contract import (
    CLIP_FRAMES,
    COMMAND_FRAME,
...
from workshop.part2.data.loader import download_dataset, load_manifest
...
def _linear(values: np.ndarray, source_time: np.ndarray) -> np.ndarray:
    ...
    return values[lower] * (1.0 - weight.reshape(shape)) + values[
        upper
...
def retime_clip(
    ...
    contact_index = np.minimum(np.rint(source_time).astype(np.int32), len(contacts) - 1)
    ...
    *,
    ...
    for parent_row in parent_manifest["sessions"]:
        ...
        keep = (
            ...
            & (arrays["joint_limit_fraction"] <= gates["joint_limit_fraction_max"])
        )
    ...
    (output_root / "README.md").write_text(
        "# Dynamically scaled Part 2 release\n\n"
...
def main() -> None:
    ...
    if args.download:
        download_dataset(args.parent_root)
    manifest = build(
        args.parent_root,
        args.output_root,
